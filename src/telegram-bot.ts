...
export class TelegramBot {
	...
	constructor() {
		...
		this.bot.onText(/\/notify ([^\s]+) ([<>=]+) ([0-9]+)/, (msg, match) => {
		...
		this.bot.onText(/\/state/, (msg, match) => {
			...
			this.sendTo(msg.chat.id, '*Current state:*' + JSON.stringify(stateManager.states, null, 4));
		});
		...
		this.bot.onText(/\/properties/, (msg, match) => {
			...
			this.getAvailableProperties()
				.subscribe( (res: any) => {
					...
					this.sendTo(msg.chat.id, 'Available properties: \n - - - \n' + message);
				})
		...
		this.bot.onText(/\/criteria/, (msg, match) => {
			...
			this.sendTo(msg.chat.id, '*Your notification criteria are:* ' + JSON.stringify(this.getConversation(msg.chat.id).subscribedTo, null, 4));
		});
		/*
	...
	getConversations() {
		...
		MongoClient.connect(DBURL, (err, client) => {
			if (err) {
				...
				db.collection('convos').find({}).toArray((err, res) => {
					if (err) {
						results.error(err);
					} else {
						results.next(res);
	...
	getAvailableProperties() {
		...
		MongoClient.connect(DBURL, (err, client) => {
			if (err) {
				...
				const db = client.db('sos-notification');
				db.collection('properties').find({}).sort({source: 1, property: 1}).toArray((err, res) => {
					if (err) {
						results.error(err);
					} else {
						results.next(res);
